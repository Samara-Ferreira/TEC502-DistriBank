...
class Bank:
    ...
    def create_pix_key(self, cpf, type_account, type_key, key):
        with self.lock:
            ...
            return f"Não foi possível criar a chave PIX!"
    ...
    def deposit(self, cpf_cnpj, type_account, key, value):
        if type_account == "physical":
            print("CHEGA AQ COM ", cpf_cnpj, value)
    ...
    def create_deposit(self, bank, cpf, type, key, value, id):
        #with self.lock_add2:
            op = {"bank": bank, "cpf": cpf, "type": type, "key": key, "value": value, "operation": "deposit", "id": id}
            retorno = self.add_operations_in_packet(op)
            return retorno

    def create_withdraw(self, bank, cpf, type, key, value, id):
        op = {"bank": bank, "cpf": cpf, "type": type, "key": key, "value": value, "operation": "withdraw", "id": id}
        self.add_operations_in_packet(op)

    # Método para criação das operações de transações
    def create_transaction(self, port_recp, port_send, cpf_recp, cpf_send, type_recp, type_send, key, value,
                                     id):
    ...
    def add_operations_in_packet(self, operation):
        with self.lock_add:
            ...
            '''print("\nLISTA 1")
            print(self.operations_package, "\n")

            print("\nLISTA 2")
            print(self.operations_package_sing, "\n")'''
        ...
        with self.lock_exec:
            for i in range(len(self.operations.queue)):
                ...
                for op in operations:
                    print("OPERAÇÃO: ", op["cpf"], op["value"], op["id"])
